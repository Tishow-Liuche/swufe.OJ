...
type Contest = {
  id: string; title: string; description?: string; mode: 'ACM' | 'IOI'; visibility: string;
...
const form = ref({ title: '', description: '', mode: 'ACM', startTime: '', endTime: '', registerStart: '', registerEnd: '', freezeTime: '', penaltyTime: 20, allowUpsolve: true, problemIds: [] as string[] });
...
async function createContest() {
  ...
  try {
    ...
    form.value = { title: '', description: '', mode: 'ACM', startTime: '', endTime: '', registerStart: '', registerEnd: '', freezeTime: '', penaltyTime: 20, allowUpsolve: true, problemIds: [] };
    await load();
    ...
    <section class="workspace">
      ...
      <section v-if="selected" class="detail">
        <header class="detail-head">
          <div>
            <div class="badges"><span class="mode">{{ selected.mode }}</span><span class="state" :class="selected.state.toLowerCase()">{{ stateText(selected.state) }}</span><span v-if="selected.participant?.isVirtual" class="virtual">虚拟参赛</span></div>
      ...
      <section v-else class="contest-overview">
        ...
        <div v-if="filtered.length" class="overview-grid">
          <button v-for="contest in filtered" :key="contest.id" class="overview-card" @click="selectContest(contest)">
            ...
            <div class="overview-meta"><span><CalendarClock :size="15" aria-hidden="true" />{{ dateText(contest.startTime) }}</span><span>{{ contest._count?.problems || contest.problems.length }} 题</span><span>{{ contest._count?.participants || 0 }} 人报名</span></div>
