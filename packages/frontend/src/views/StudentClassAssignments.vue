...
const assignmentStatus = computed(() => {
  ...
  if (assignment.progress?.status) {
    ...
    const tone =
      status === 'COMPLETED' || status === 'SETTLED' ? 'ok'
        : status === 'LATE' || status === 'IN_PROGRESS' ? 'pending'
          : 'muted';
    return { label: assignment.progress.statusLabel || status, tone };
  }
  const now = Date.now();
  if (new Date(assignment.startTime).getTime() > now) return { label: '尚未开始', tone: 'muted' };
  if (new Date(assignment.endTime).getTime() < now) {
      ...
      <main v-if="selectedAssignment" class="assignment-detail">
        <header class="detail-heading">
          ...
          <div class="completion-badge" :class="{ complete: selectedAssignment.progress.completed }">
            ...
            <span>
              {{ selectedAssignment.progress.solvedCount }}/{{ selectedAssignment.progress.totalProblems }}
              <small v-if="selectedAssignment.progress.score != null">得分 {{ selectedAssignment.progress.score }}</small>
            </span>
          </div>
        </header>
        ...
        <section class="problem-section">
          ...
          <ol class="problem-list">
            <li v-for="problem in selectedAssignment.problems" :key="problem.id" :class="{ solved: problem.solved }">
              <span class="problem-order">{{ problem.order }}</span>
              <router-link :to="`/problems/${problem.id}`">
                <b>{{ problem.title }}</b>
                <small>
                  {{ problem.source || 'LOCAL' }}
                  <template v-if="problem.difficulty"> · {{ problem.difficulty }}</template>
                </small>
              </router-link>
              <div class="problem-meta">
                <span v-if="problem.solved" class="status-pill" :class="problem.late ? 'pending' : 'ok'">
                  {{ problem.late ? '补交通过' : '已通过' }}
                </span>
                <span class="problem-score">{{ problem.earnedScore ?? problem.score }} 分</span>
              </div>
            </li>
          </ol>
        </section>
