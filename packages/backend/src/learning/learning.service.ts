...
export class LearningService {
  ...
  async getContinueLearning(userId: string) {
    const [wrongItems, acceptedGroups, attemptedGroups, drafts] = await Promise.all([
      this.prisma.userWrongBook.findMany({
        where: { userId },
        include: { problem: { select: problemSummary } },
    ...
    const acceptedIds = new Set(acceptedGroups.map((item) => item.problemId));
    const wrongIds = new Set(wrongItems.map((item) => item.problemId));
    const activity = new Map<string, Date>();
    for (const submission of attemptedGroups) {
      if (submission._max.createdAt) activity.set(submission.problemId, submission._max.createdAt);
    }
