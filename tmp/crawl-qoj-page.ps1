param([int]$Page = 2, [int]$Skip = 0, [int]$Take = 50)
$outFile = 'tmp/qoj-crawl.jsonl'
function Get-Mirror($url) {
  $mirror = 'https://r.jina.ai/http://r.jina.ai/http://' + $url
  for ($attempt = 1; $attempt -le 3; $attempt++) {
    try { return (Invoke-WebRequest -Uri $mirror -UseBasicParsing -TimeoutSec 45).Content }
    catch {
      if ($_.Exception.Message -match '429' -and $attempt -lt 3) { $wait = 8 * $attempt; Write-Output "429, wait ${wait}s..."; Start-Sleep -Seconds $wait } else { throw }
    }
  }
}
...
foreach ($m in $matches) {
  $id = $m.Groups[1].Value; $listTitle = $m.Groups[2].Value
  try {
